...
export function handleCurrentPoints(state, action) {
    switch (action.type) {
        ...
        case 'SUBTRACT':
            return { ...state, sum: 0, [action.payload.color]: { sum: 0, points: [] } }
        case 'CLEAR':
            return {
                b: { sum: 0, points: [] },
                r: { sum: 0, points: [] },
    ...
    switch (action.type) {
        case 'CHANGE': {
            ...
            if (!isNaN(e.target.value) && e.target.value !== '') {
                if (Math.abs(e.target.value) < 999) {
                    return { ...state, change: Math.abs(e.target.value) }
                } else {
                    return { ...state, change: 999 }
                }
        ...
        case 'SET': {
            ...
            if (
                ...
                return { ...state, set: Math.abs(e.target.value), change: Math.abs(e.target.value) }
            } else if (e.target.value > pointsLookup[lvl].max) {
                return { ...state, set: pointsLookup[lvl].max, change: pointsLookup[lvl].max }
            } else if (e.target.value < pointsLookup[lvl].min) {
                return { ...state, set: pointsLookup[lvl].min, change: pointsLookup[lvl].min }
            } else {
                return state
            }
        }
        ...
        case 'INITIALIZE': {
            ...
            if (localStorage.getItem('maxPoints')) {
                if (
                    Math.abs(localStorage.getItem('maxPoints')) <= pointsLookup[lvl].max ||
...
export function handleWarning(state, action) {
    switch (action.type) {
        case 'RESET-WARNING':
            return { ...state, resetWarning: !state.resetWarning }
        default:
            throw new Error(`action.type: ${action.type} is not a valid action.`)
    }
}
...
export function handleError(state, action) {
    switch (action.type) {
        case 'MAX-POINTS-ERROR':
            return { ...state, maxPointsErrorNotification: !state.maxPointsErrorNotification }
        default:
            throw new Error(`action.type: ${action.type} is not a valid action.`)
    }
}
