...
const Content = () => {
    ...
    function getSize() {
        try {
            ...
            for (let i in refHeaderTraits.current) {
                sizeHeaderTraits = Object.assign(Object.assign({}, sizeHeaderTraits), { [i]: new maxElementSize(refHeaderTraits.current[i].children) });
            }
            const sizeTree = new maxElementSize(refTree.current);
            let gridReps = {};
            for (let i in sizeHeaderTraits) {
                let arr = [];
                const widthRes = sizeTree.width -
                ...
                gridReps = Object.assign(Object.assign({}, gridReps), { [i]: { gridTemplateColumns: arr.join(' ') } });
            }
    ...
    return (<div className="content">
            <ErrorBoundary>
                <Tree treeClass={treeClass} treeColor="b" treeRef={refHeaderTraits} treeTraitStyle={treeTraitStyle}/>
            </ErrorBoundary>
            <ErrorBoundary>
                <Tree treeClass={treeClass} treeColor="r" treeRef={refHeaderTraits} treeTraitStyle={treeTraitStyle}/>
            </ErrorBoundary>
            <ErrorBoundary>
                <Tree treeClass={treeClass} treeColor="y" treeRef={refHeaderTraits} treeTraitStyle={treeTraitStyle}/>
            </ErrorBoundary>
        </div>);
};
