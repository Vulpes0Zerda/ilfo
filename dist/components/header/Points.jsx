...
const Points = () => {
    const { currentPoints, dispatchCurrentPoints, maxPoints, dispatchMaxPoints } = useContext(PointContext);
    const { level } = useContext(LevelContext);
    const maxPointsRef = useRef();
    return (<div className="header__pts" onClick={() => {
            maxPointsRef.current.focus();
        }}>
            <span>{currentPoints.sum}</span>
            <span>{' / '}</span>
            <input value={maxPoints.change} type={'string'} ref={maxPointsRef} onBlur={(e) => {
            dispatchMaxPoints({
                type: 'SET',
